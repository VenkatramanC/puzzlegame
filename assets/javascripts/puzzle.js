...
var newArray = [];                    // var images is array of image pieces 
...
function shuffle(images) {           
  ...
  while (0 !== currentIndex) {
    ...
    newImages[currentIndex] = images[randomIndex];
    newImages[randomIndex] = temporaryValue;
  }
...
$(document).ready(function(){
  ...
  $("#submit").click(function(){                  //when we click on submit
    //console.log(newArr)
    var count = 0;
    var actualArr = $(".sortable").sortable('toArray');
    var newArr = $.map($('.puzzle .col-md-4'), function(ele, i) {return $(ele).attr('id')})
    for (var i = 0 ; i < images.length ; i++){
      if (actualArr[i] == newArr[i]) {              //comparing resultArray with shuffled images array
        count += 1;
      }
    }
    debugger
  ...
  $("#reload").click(function(){
    ...
    //console.log(images);
    $("#a1 img").attr("src", images[0])
    $("#a2 img").attr("src", images[1])
    $("#a3 img").attr("src", images[2])
    $("#a4 img").attr("src", images[3])
    $("#a5 img").attr("src", images[4])
    $("#a6 img").attr("src", images[5])
    $("#a7 img").attr("src", images[6])
    $("#a8 img").attr("src", images[7])
    $("#a9 img").attr("src", images[8])
  });
});
